...
# URL-API ------------------------------------------------------------------------------------------
apikey <- "&CMC_PRO_API_KEY=6f8eb99f-1de9-4566-ae06-b423c3d9fe15"
...
URL2 <- paste0(cmcURL, "convert=USD&symbol=BTC", apikey)
URL2 <- gsub(" ", "", URL2)

js2 <- fromJSON(URL2)$data

BTC_USD <- js2$BTC$quote$USD$price
BTC_EUR <- js$BTC$quote$EUR$price
...
currentTime <- format(currentTime, "%d-%m-%Y at %H:%M")
```
<div class="footer">`r paste0(greyspan, currentTime, "</span>")`</div>


<div class="footer">`r paste0(greyspan, format(`attr<-`(Sys.time(),"tzone","CET"), "%d-%m-%Y at %H:%M"), "</span>")`</div>
