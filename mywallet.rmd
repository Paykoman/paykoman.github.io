...
# https://next-game-solutions.github.io/geckor/       CHECK THIS ONE!!!!!!!!
...
if (params$user == "claudine") W_EUR <- fread("wallet_claudine.csv")
if (params$user == "total") W_EUR <- fread("wallet_total.csv")
```
...
# Get prices ---------------------------------------------------------------------------------------
PRICES <- current_price(W_EUR$coin_id_geckor, vs_currencies = c("usd", "eur"), FALSE, FALSE, FALSE) %>% 
  data.table

# 1 EUR = 1 USDEUR
USDEUR <- PRICES[coin_id == "bitcoin", price[vs_currency == "eur"]/price[vs_currency == "usd"]] 
USDEUR <- round(USDEUR, 2)


# Holdings -----------------------------------------------------------------------------------------
W_EUR[, coins_initial := wallet_initial/hold]
W_EUR[, coins_current := PRICES[coin_id == coin_id_geckor & vs_currency == "eur", price], coin_id_geckor]
